...
router.post(
    ...
    asyncHandler(async (req, res) => {
        ...
        if (existing) throw ApiError.conflict("Email already taken");
...
router.post(
    ...
    asyncHandler(async (req, res) => {
        ...
        if (!ok) throw ApiError.notFound("Invalid credentials");
...
router.patch(
    ...
    asyncHandler(async (req, res) => {
        ...
        if (!user) throw ApiError.notFound("Session no longer valid");

        const ok = await user.comparePassword(req.body.currentPassword);
        if (!ok) throw ApiError.notFound("Current Password is incorrect");
